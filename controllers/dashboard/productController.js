...
class productController {
    add_product = async(req,res) => {
        ...
        form.parse(req, async(err,field,files) => {
            let {name,category,description,stock,discount,price,shopName,brand} = field
            const {images} = files
            name = name.trim()
    ...
    product_update = async (req,res) => {
        let {name,description,stock,discount,price,productId,brand} = req.body
        name = name.trim()
        const slug = name.split(' ').join('-')
        try {
            await productModel.findByIdAndUpdate(productId, {
                name,description,stock,discount,price,brand,productId,slug
            })
            const product = await productModel.findById(productId)
